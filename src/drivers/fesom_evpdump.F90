program fesom_evpdump
    ! M3b sea-ice EVP-dynamics byte-gate driver. Mirrors fesom_icedump (M3a) init up to
    ! ice_setup (mesh + geometry + ALE depths + do_ic3d ocean IC + ssh_stiff CSR + the
    ! cold-start ice IC), then PRESCRIBES an analytic ocean-forcing state (surface ocean
    ! velocity dyn%uv(:,1,:), elevation mesh%hbar, wind-on-ice stress stress_atmice_x/y —
    ! identical formulas to the FESOM2 oracle shim src/fesom_ice_dump.F90::evp_dump_write,
    ! from the byte-identical rotated coords), runs ocean2ice (-> srfoce_u/v/temp/salt/ssh)
    ! + EVPdynamics (-> uice/vice + sigma11/12/22 after 120 subcycles), and dumps them in
    ! the oracle's FADVHDMP format. tools/run_evp_gate_core2.sh compares vs the oracle for
    ! max|delta|=0, CORE2 1-rank.
    !
    ! cd_oce_ice / delta_min are set to the CORE2 namelist.ice DOUBLES (0.0055_WP /
    ! 1.0e-11_WP), NOT the t_ice single->WP defaults — the M3a precision note (the oracle's
    ! namelist read parses the literals as doubles).
    !
    !   FESOM3_MESH_DIR  mesh dir   (default: CORE2)
    !   FESOM3_IC_FILE   IC netcdf  (default: pool phc3.0_winter.nc — same file the oracle reads)
    !   FESOM3_EVP_OUT   out path   (default: evp_f3.bin)
    use mpi
    ...
    integer :: nsw, n, nz, nl, nzmin, nzmax, e, u, whichevp, ios
    ...
    call get_environment_variable('FESOM3_EVP_OUT', out_path)
    if (len_trim(out_path) == 0) out_path = 'evp_f3.bin'
    ...
    if (partit%npes /= 1) then
        if (partit%mype == 0) write(*,'(a)') 'fesom_evpdump: requires 1 rank'
    ...
    nl = mesh%nl
    write(*,'(a,i0,a,i0,a,i0,a,i0,a,i0)') 'fesom_evpdump: nod2D=', mesh%nod2D, &
    ...
    ! eta=0 — identical to fesom_icedump.
    ...
    write(*,'(a,2es12.4)') 'fesom_evpdump: surface T range = ', &
    ...
    ! deformation params to the CORE2 namelist DOUBLES (M3a precision note).
    ...
    ice%delta_min  = 1.0e-11_WP       ! namelist delta_min  (double), not the 1.0e-11  single->WP default
    write(*,'(a,i0,a,3es12.4)') 'fesom_evpdump: whichEVP=', whichevp, &
        '  max a_ice/m_ice/m_snow = ', &
    ...
    ! MUST equal the FESOM2 oracle src/fesom_ice_dump.F90::evp_dump_write.
    do n = 1, mesh%nod2D
    ...
    !===========================================================================
    ! ocean -> ice coupling + EVP dynamics (120 subcycles, whichEVP dispatched).
    call ocean2ice(ice, dyn, tracers, mesh)
    call EVPdynamics_solve(ice, mesh)
    write(*,'(a,2es12.4)') 'fesom_evpdump: max|uice| max|vice| = ', &
        maxval(abs(ice%uice)), maxval(abs(ice%vice))
    write(*,'(a,3es12.4)') 'fesom_evpdump: max|sig11| |sig12| |sig22| = ', &
        maxval(abs(ice%work%sigma11)), maxval(abs(ice%work%sigma12)), maxval(abs(ice%work%sigma22))

    !===========================================================================
    ! dump (FADVHDMP, 1-rank identity ids). srfoce_u/v + uice/vice on nod2D; sigma on elem2D.
    call advhor_dump_open(u, trim(out_path), mesh%nod2D, mesh%elem2D, mesh%edge2D, nl)
    call wr_r1(u, 'ice_srfoce_u', ice%srfoce_u(1:mesh%nod2D))
    call wr_r1(u, 'ice_srfoce_v', ice%srfoce_v(1:mesh%nod2D))
    call wr_r1(u, 'ice_uice',     ice%uice(1:mesh%nod2D))
    call wr_r1(u, 'ice_vice',     ice%vice(1:mesh%nod2D))
    call wr_r1(u, 'ice_sigma11',  ice%work%sigma11(1:mesh%elem2D))
    call wr_r1(u, 'ice_sigma12',  ice%work%sigma12(1:mesh%elem2D))
    call wr_r1(u, 'ice_sigma22',  ice%work%sigma22(1:mesh%elem2D))
    call advhor_dump_close(u)
    write(*,'(a)') 'fesom_evpdump: wrote '//trim(out_path)

    call par_ex(partit%MPI_COMM_FESOM, partit%mype)
end program fesom_evpdump
