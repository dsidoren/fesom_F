...
contains
    ...
    subroutine muscl_adv_init(twork, mesh)
        ...
        if (.not. allocated(twork%nboundary_lay)) allocate(twork%nboundary_lay(mesh%nod2D))
    ...
    subroutine find_up_downwind_triangles(twork, mesh)
        ...
        ! the atan2 angles. 1-rank simplification per module header.
        type(t_mesh),        intent(in)    :: mesh
        type(t_tracer_work), intent(inout) :: twork
        integer       :: n, k, ednodes(2), elem
        real(kind=WP) :: x(2), b(2), c(2), cr, bx, by, xx, xy, ab, ax, cl

        if (.not. allocated(twork%edge_up_dn_tri))  allocate(twork%edge_up_dn_tri(2, mesh%edge2D))
        if (.not. allocated(twork%edge_up_dn_grad)) allocate(twork%edge_up_dn_grad(4, mesh%nl-1, mesh%edge2D))
        ...
        do n = 1, mesh%edge2D
            ...
            do k = 1, mesh%nod_in_elem2D_num(ednodes(1))
                ...
                call tri_sides(mesh, elem, ednodes(1), b, c)
            ...
            do k = 1, mesh%nod_in_elem2D_num(ednodes(2))
                ...
                call tri_sides(mesh, elem, ednodes(2), b, c)
                if (b(1) >  cl/2.0_WP) b(1) = b(1) - cl
                if (b(1) < -cl/2.0_WP) b(1) = b(1) + cl
                if (c(1) >  cl/2.0_WP) c(1) = c(1) - cl
                if (c(1) < -cl/2.0_WP) c(1) = c(1) + cl
    ...
    !---------------------------------------------------------------------------
    subroutine fill_up_dn_grad(twork, tr_xy, mesh)
